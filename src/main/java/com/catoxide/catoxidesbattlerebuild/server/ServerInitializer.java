...
import com.catoxide.catoxidesbattlerebuild.network.AnimationSyncLogger;
 ...
 * 2. UnifiedModelDataManager - 统一模型数据管理器（依赖ServerConfig）
 * 3. ServerGeoModelManager - 模型管理器（依赖ServerConfig, UnifiedModelDataManager）
 * 4. ServerEntityManager - 实体管理器（依赖ServerGeoModelManager, UnifiedModelDataManager）
 * 5. EntityCollectionFactory - 实体集合工厂（依赖ServerGeoModelManager, ServerEntityManager）
 * 6. ServerSideExecutor - 服务端执行器（依赖上述所有）
 * 7. MatrixTransformer - 矩阵变换器（独立，可选）
 * 8. ModelCollectionFactory - 模型集合工厂（依赖ServerGeoModelManager）
 * 
 * @see ServerConfig
 * @see ModelDataManager
...
public class ServerInitializer {
    ...
    public enum InitializationPhase {
        CONFIG("配置初始化"),
        UNIFIED_MODEL_MANAGER("统一模型数据管理器"),
        MODEL_MANAGER("模型管理器"),
    ...
    private static void initializeServerComponents() {
        ...
        AnimationSyncLogger.logInitializationStart();
        ...
        try {
            ...
            initializePhase(InitializationPhase.CONFIG, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.CONFIG.name(), "配置系统初始化完成", true);
            });

            // 阶段2: 统一模型数据管理器
            initializePhase(InitializationPhase.UNIFIED_MODEL_MANAGER, () -> {
                ModelDataManager.getInstance();
                GeckoLib.LOGGER.info("✓ 统一模型数据管理器初始化完成");
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.UNIFIED_MODEL_MANAGER.name(), "统一模型数据管理器初始化完成", true);
            });
            ...
            initializePhase(InitializationPhase.MODEL_MANAGER, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.MODEL_MANAGER.name(), "模型管理器初始化完成", true);
            });
            ...
            initializePhase(InitializationPhase.ENTITY_MANAGER, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.ENTITY_MANAGER.name(), "实体管理器初始化完成", true);
            });
            ...
            initializePhase(InitializationPhase.COLLECTION_FACTORY, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.COLLECTION_FACTORY.name(), "实体集合工厂初始化完成", true);
            });
            ...
            initializePhase(InitializationPhase.EXECUTOR, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.EXECUTOR.name(), "服务端执行器初始化完成", true);
            });
            ...
            initializePhase(InitializationPhase.UTILITIES, () -> {
                ...
                AnimationSyncLogger.logInitializationPhase(InitializationPhase.UTILITIES.name(), "工具类初始化完成", true);
            });

            // 完成
            long duration = System.currentTimeMillis() - startTime;
            GeckoLib.LOGGER.info("=== 服务器组件初始化成功 (耗时: {}ms) ===", duration);
            AnimationSyncLogger.logInitializationComplete(true, duration);

        } catch (Exception e) {
            long duration = System.currentTimeMillis() - startTime;
            GeckoLib.LOGGER.error("=== 服务器组件初始化失败 ===", e);
            AnimationSyncLogger.logInitializationComplete(false, duration);
            AnimationSyncLogger.logError("服务器组件初始化失败", e);
    ...
    private static void initializePhase(InitializationPhase phase, Runnable initializer) {
        ...
        AnimationSyncLogger.logInitializationPhaseStart(phase.name(), phase.getDescription());
        ...
        try {
            ...
            AnimationSyncLogger.logInitializationPhaseEnd(phase.name(), phaseDuration);
        } catch (Exception e) {
            long phaseDuration = System.currentTimeMillis() - phaseStart;
            GeckoLib.LOGGER.error("[{}] 失败", phase.name(), e);
            AnimationSyncLogger.logInitializationPhaseEnd(phase.name(), phaseDuration);
            AnimationSyncLogger.logError("初始化阶段失败: " + phase.name(), e);
            throw e;
        }
    }
    ...
    public static boolean isInitialized() {
        try {
            ...
            AnimationSyncLogger.logError("系统未完全初始化", e);
            return false;
        }
    }
    ...
    public static String getInitializationStatus() {
        ...
        try {
            ModelDataManager.getInstance();
            status.append("✓ UnifiedModelDataManager: 已初始化\n");
        } catch (Exception e) {
            status.append("✗ UnifiedModelDataManager: 未初始化\n");
        }
